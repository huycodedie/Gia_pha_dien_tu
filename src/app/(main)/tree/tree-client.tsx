...
function computeBranchSummary(
  ...
  function walk(h: string, gen: number) {
    ...
    if (person.isLiving) livingCount++;
    else deceasedCount++;
  ...
  if (person) {
    for (const fId of person.families) {
      ...
      if (
        fam.motherHandle &&
        fam.motherHandle !== handle &&
        !visited.has(fam.motherHandle)
      ) {
        const spouse = personMap.get(fam.motherHandle);
        if (spouse) {
          visited.add(fam.motherHandle);
          if (spouse.isLiving) livingCount++;
          else deceasedCount++;
        }
      }
      if (
        fam.fatherHandle &&
        fam.fatherHandle !== handle &&
        !visited.has(fam.fatherHandle)
      ) {
        const spouse = personMap.get(fam.fatherHandle);
        if (spouse) {
          visited.add(fam.fatherHandle);
          if (spouse.isLiving) livingCount++;
          else deceasedCount++;
        }
      }
  ...
  return {
    ...
    generationRange: [
      minGen === Infinity ? 0 : minGen,
      maxGen === -Infinity ? 0 : maxGen,
    ],
    livingCount,
    deceasedCount,
    patrilinealCount,
...
function computeTreeStats(
  ...
  for (const n of nodes) {
    ...
    if (n.node.isLiving) living++;
    else deceased++;
    if (n.node.isPatrilineal) patri++;
    else nonPatri++;
  }
...
export default function TreeViewPage() {
  ...
  const [addSpousePerson, setAddSpousePerson] = useState<TreeNode | null>(null);
  const [addChildFamily, setAddChildFamily] = useState<string | null>(null);
  ...
  const [hasOwnTree, setHasOwnTree] = useState(false);
  ...
  useEffect(() => {
    const checkOwnTree = async () => {
      ...
      try {
        ...
        setHasOwnTree(Boolean(ownPeople && ownPeople.length > 0));
      } catch (error) {
        console.error("Error checking own tree:", error);
      }
    };
  ...
  // Add first person to user's family tree
  const addFirstPerson = useCallback(async () => {
    if (!profile) return;
    ...
    try {
      ...
      // Create first person
      const firstPerson = {
        displayName: profile.display_name || "Người đầu tiên",
        gender: 1, // Male
        generation: 1,
        birthYear: undefined,
        deathYear: undefined,
  ...
  const zoomLevel = useMemo<ZoomLevel>(
    () => getZoomLevel(transform.scale),
    [transform.scale],
  );

  // F4: Get all descendants of collapsed branches
  const getDescendantHandles = useCallback(
    (handle: string): Set<string> => {
      ...
      function walk(h: string) {
        ...
        for (const fId of person.families) {
          ...
          if (!fam) continue;
          // Include spouse
          if (fam.motherHandle && fam.motherHandle !== h)
            result.add(fam.motherHandle);
          if (fam.fatherHandle && fam.fatherHandle !== h)
    ...
    while (changed) {
      ...
      for (const p of treeData.people) {
        ...
        const allParentsHidden = p.parentFamilies.every((pfId) => {
          ...
          const fatherHidden = pf.fatherHandle
            ? hidden.has(pf.fatherHandle)
            : true;
          const motherHidden = pf.motherHandle
            ? hidden.has(pf.motherHandle)
            : true;
  ...
  const layout = useMemo<LayoutResult | null>(() => {
    ...
    const visiblePeople = d.people.filter(
      (p: TreeNode) => !hiddenHandles.has(p.handle),
    );
    const visibleFamilies = d.families.filter((f: TreeFamily) => {
      // Keep family only if NOT all parents are hidden
      const fatherHidden = f.fatherHandle
        ? hiddenHandles.has(f.fatherHandle)
        : true;
      const motherHidden = f.motherHandle
        ? hiddenHandles.has(f.motherHandle)
        : true;
      return !(fatherHidden && motherHidden);
    });
  ...
  const visibleHandles = useMemo(
    () => new Set(visibleNodes.map((n) => n.node.handle)),
    [visibleNodes],
  );
  ...
  const { parentPaths, couplePaths, visibleCouples } = useMemo(() => {
    if (!layout)
      return {
        parentPaths: "",
        couplePaths: "",
        visibleCouples: [] as PositionedCouple[],
      };
    let pp = "";
    let cp = "";
  ...
  // Copy shareable link
  const copyTreeLink = useCallback(async (handle: string) => {
    ...
    try {
      if (typeof navigator !== "undefined" && navigator.clipboard) {
        ...
        // Fallback: could show a toast or just set copied state
        setLinkCopied(true);
        setTimeout(() => setLinkCopied(false), 2000);
      }
    } catch (error) {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-80px)]">
            ...
            <Button
              ...
              onClick={() =>
                setTransform((t) => {
                  ...
                  const cx = vw / 2;
                  const cy = vh / 2;
                  const ns = Math.min(t.scale * 1.3, 3);
                  const r = ns / t.scale;
                  return {
                    scale: ns,
                    x: cx - (cx - t.x) * r,
            ...
            <Button
              ...
              onClick={() =>
                setTransform((t) => {
                  ...
                  const cx = vw / 2;
                  const cy = vh / 2;
                  const ns = Math.max(t.scale / 1.3, 0.15);
                  const r = ns / t.scale;
                  return {
                    scale: ns,
                    x: cx - (cx - t.x) * r,
            ...
            {profile &&
              ...
              (profile.role === "user" || profile.role === "admin") && (
                <Button
                  ...
                  title="Thêm người đầu tiên vào gia phả"
                >
      ...
      <div className="flex-1 flex gap-0 min-h-0">
        <div
          ...
          onClick={() => {
            setShowSearch(false);
            setContextMenu(null);
            if (editorMode) setSelectedCard(null);
          }}
        >
          {loading ? (
            ...
            layout && (
              ...
              >
                ...
                {Array.from(branchSummaries.entries()).map(
                  ([handle, summary]) => {
                    const parentNode = layout.nodes.find(
                      (n) => n.node.handle === handle,
                    );
                ...
                {contextMenu &&
                  (() => {
                    const person = treeData?.people.find(
                      (p) => p.handle === contextMenu.handle,
                    );
                    ...
                    return (
                      <CardContextMenu
                        ...
                        profile={profile}
                        onViewDetail={() => {
                          router.push(`/people/${person.handle}`);
                          setContextMenu(null);
                        }}
                        onShowDescendants={() => {
                          setFocusPerson(person.handle);
                          setViewMode("descendant");
                          setContextMenu(null);
                        }}
                        onShowAncestors={() => {
          ...
          <div className="absolute bottom-2 left-2 bg-background/80 backdrop-blur border rounded px-1.5 py-0.5 text-[10px] text-muted-foreground flex gap-1.5">
            ...
            {layout && (
              <span>
                {visibleNodes.length}/{layout.nodes.length} nodes
              </span>
            )}
          </div>
      ...
      {addChildFamily && (
        <AddChildDialog
          familyHandle={addChildFamily}
          onClose={() => setAddChildFamily(null)}
          onSuccess={() => {
            setAddChildFamily(null);
            // Refresh tree data
            fetchTreeData().then(setTreeData);
          }}
        />
  ...
  if (zoomLevel === "mini") {
    return (
      <div
        ...
        onClick={(e) => {
          e.stopPropagation();
          onClick(node.handle, x + CARD_W, y + CARD_H / 2);
        }}
      >
        <div
          className={`w-3 h-3 rounded-full shadow-sm ${node.imageUrl ? "bg-cover bg-center" : ""}`}
          style={{
            backgroundColor: node.imageUrl ? undefined : dotColor,
            ...(node.imageUrl
          ...
          <div className="relative flex-shrink-0">
            <div
              className={`w-7 h-7 rounded-full flex items-center justify-center
                          ...
                          ${node.imageUrl ? "bg-cover bg-center" : ""}`}
              {...(node.imageUrl
                ? { style: { backgroundImage: `url(${node.imageUrl})` } }
                : {})}
            >
              {!node.imageUrl && initials}
            </div>
          </div>
          <div className="flex-1 min-w-0">
          ...
          {isPatri && (
            <span
              className="absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full bg-gradient-to-br from-teal-400 to-emerald-500
                            text-white text-[4px] flex items-center justify-center shadow-sm font-bold ring-1 ring-white"
            >
...
function BranchSummaryCard({
      ...
      >
        <div className="w-4 h-4 rounded bg-amber-400 shadow-sm flex items-center justify-center">
          <span className="text-[7px] text-white font-bold">
            {summary.totalDescendants}
          </span>
        </div>
        <div
          className="hidden group-hover:block absolute -top-10 left-1/2 -translate-x-1/2 z-50
                    bg-slate-900 text-white text-[10px] px-2 py-1 rounded shadow-lg whitespace-nowrap pointer-events-none"
        >
          📦 {summary.totalDescendants} người · Đời {summary.generationRange[0]}
          →{summary.generationRange[1]}
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="px-2.5 py-2 h-full flex items-center gap-2.5">
        <div
          className="w-11 h-11 rounded-full bg-gradient-to-br from-amber-400 to-orange-500
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-0.5 flex items-center gap-1.5 text-[9px]">
            ...
            <span className="text-amber-600 ml-auto text-[8px] font-medium">
              ▶ Mở
            </span>
          </div>
        </div>
...
function GenerationHeaders({
  ...
  const entries = Array.from(generationStats.entries()).sort(
    (a, b) => a[0] - b[0],
  );
...
function StatsOverlay({
  ...
  return (
    <div
      className="absolute top-3 right-3 w-64 bg-white/95 backdrop-blur-lg border border-slate-200
...
function EditorPanel({
  ...
  const [newChildSearch, setNewChildSearch] = useState(""); //
  const [showChildDropdown, setShowChildDropdown] = useState(false); //
  const newChildSearchRef = useRef<HTMLDivElement>(null); //

  if (!treeData) return null;

  const person = selectedCard
  ...
  // ➕ useMemo tìm tất cả người KHÔNG phải con hiện tại
  const availableChildren = useMemo(() => {
    if (!treeData || !parentFamily || !person) return [];
    return treeData.people.filter(
      (p) =>
        !parentFamily.children.includes(p.handle) &&
        p.handle !== person.handle &&
        p.isPatrilineal, // Optional: chỉ chính tộc
    );
  }, [treeData, parentFamily, person?.handle]);

  const filteredAvailableChildren = useMemo(() => {
    if (!newChildSearch.trim()) return availableChildren.slice(0, 10);
    const q = newChildSearch.toLowerCase();
    return availableChildren
      .filter((p) => p.displayName.toLowerCase().includes(q))
      .slice(0, 10);
  }, [availableChildren, newChildSearch]);
    ...
    <div className="w-72 bg-background border-l flex flex-col overflow-hidden flex-shrink-0">
      ...
      {!person ? (
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {/* Children management - MỚI HOÀN TOÀN */}
          {parentFamily && (
            <div className="p-3 border-b space-y-3">
              <div className="flex items-center justify-between">
                ...
                {/* ➕ NÚT THÊM CON */}
                <button
                  className="px-2 py-1 text-xs bg-green-100 hover:bg-green-200 text-green-800 rounded font-medium transition-all"
                  onClick={() => setShowChildDropdown(true)}
                >
                  Thêm
                </button>
              </div>
              ...
              {/* ➕ SEARCH THÊM CON MỚI */}
              <div ref={newChildSearchRef} className="pt-2 border-t">
                ...
                <div className="relative">
                  <input
                    type="text"
                    className="w-full border rounded px-2 py-1.5 text-xs bg-background placeholder:text-muted-foreground/60 pr-8"
                    placeholder="🔍 Tìm người để thêm làm con..."
                    value={newChildSearch}
                    onChange={(e) => {
                      setNewChildSearch(e.target.value);
                  ...
                  {/* DROPDOWN NGƯỜI CÒN LẠI */}
                  {showChildDropdown && (
                    <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-background border rounded-lg shadow-xl max-h-40 overflow-y-auto">
                      {filteredAvailableChildren.length === 0 ? (
                        ...
                        filteredAvailableChildren.map((child) => (
                          <button
                            key={child.handle}
                            className="w-full text-left px-2 py-1.5 text-sm hover:bg-emerald-50 border-b border-muted/50 last:border-b-0 flex items-center gap-3 transition-all group hover:shadow-sm"
                            onClick={() => {
                              // ➕ CHECK: Con đã có cha khác chưa?
                              const existingParentFamily =
                                treeData.families.find(
                                  (f) =>
                                    f.children.includes(child.handle) &&
                                    f.handle !== parentFamily.handle, // Trừ family hiện tại
                                );

                              if (existingParentFamily) {
                                // CẢNH BÁO: Đã có cha khác
                                if (
                                  !confirm(
                                    `"${child.displayName}" đã là con của "${
                                      treeData.people.find(
                                        (p) =>
                                          p.handle ===
                                            existingParentFamily.fatherHandle ||
                                          p.handle ===
                                            existingParentFamily.motherHandle,
                                      )?.displayName
                                    }".\n\n` +
                                      `Bạn có muốn CHUYỂN "${child.displayName}" từ gia đình cũ sang ` +
                                      `gia đình "${person.displayName}" không?`,
                                  )
                                ) {
                                  return; // Hủy
                                }

                                // ➕ CHUYỂN từ family cũ sang family mới
                                onMoveChild(
                                  child.handle,
                                  existingParentFamily.handle,
                                  parentFamily.handle,
                                );
                              } else {
                                // Thêm vào cuối danh sách con
                                const newOrder = [
                                  ...parentFamily.children,
                                  child.handle,
                                ];
                          ...
                          >
                            ...
                            {/* <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shadow-sm
                                                            ${child.gender === 1 ? 'bg-blue-500 text-white' 
                                                            : child.gender === 2 ? 'bg-pink-500 text-white'
                                                            : 'bg-gray-400 text-white'}`}>
                                                            {child.displayName.split(' ').map(w => w[0]).join('')}
                                                        </div> */}
                            ...
                            {/* <div className="flex-1 min-w-0">
                                                            <span className="font-medium truncate">{child.displayName}</span>
                                                            <div className="text-[10px] text-muted-foreground flex items-center gap-1">
                                                                <span>{child.handle}</span>
                                                                <span>Đời {child.generation}</span>
                                                            </div>
                                                        </div> */}
          ...
          {/* Children reorder - hiện danh sách con cũ*/}
